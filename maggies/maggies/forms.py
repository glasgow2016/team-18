...
class VisitorForm(forms.Form):
    ...
    is_new_visitor = forms.BooleanField(required=False, label="First visit?", help_text="Has this person visited before today?")
    visitor_gender = forms.ChoiceField(choices=GENDER_CHOICES, label="Gender")
    nature_of_visit = forms.ModelChoiceField(VisitNature.objects.all(), label="Nature of Visit", help_text="Why did this person visit today?")
    cancer_site = forms.ModelChoiceField(CancerSite.objects.all(), label="Cancer Site", help_text="What type of cancer does the person deal with? Select 'Not Stated' if it did not come up in conversation, or 'Unknown Primary' if even their doctor doesn't know.")
